...
static void add (const int & in, int & out)
{
   out = in + in;
   ...
   try
   {
      ...
      if (opt_server)
      {
         ...
         RPCHandler tmp = boost::bind (rpcServerHelper<int,int>,
               boost::function<void(const int &, int&)>(add), _1, _2, _3);

         // Register RPC handlers

         rpcserver->registerRPC ("user.add",
               boost::bind (&threadRPC, tmp, _1, _2, _3));

         rpcserver->registerRPC ("user.null",
               boost::bind (&threadRPC, rpc_null, _1, _2, _3));
      }
      ...
      if (!opt_remote.empty())
      {
         ...
         {
            testManual (rpcclient->rpcConnect ("user.null", addr));
         }
         ...
         for (size_t i=0; i<10; ++i)
         {
            ...
            in = i;
            rpcClientHelper (in, out, rpcclient->rpcConnect ("user.add", addr));
            ZLOG_INFO (log_, format ("user.add: in=%i out=%i") % in % out);
         }
